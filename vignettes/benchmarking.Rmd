...
se_base_mean <- function() {
  ...
  se_object_cell_type_counts <- t(rowsum(t(se_object_cell_type_counts), group = colnames(se_object_cell_type_counts)))
  colSums(se_object_counts_cell_type)
} 
```
...
operation <- 
  c(
    "Aggregate cells by variable", "Plot feature counts by cell", "Subset cells by feature signature",
    "Aggregate cells by variable", "Plot feature counts by cell", "Subset cells by feature signature",
    "Aggregate cells by variable", "Plot feature counts by region", "Subset cells by feature signature",
    "Sum feature counts by cell type", "Plot feature counts by condition", "Split sample metadata column",
    ...
    "Downsample cells", "Aggregate cells by variable", "Normalise features", "Subset cells by feature signature", "Plot PCA" 
  ) |>
  rep(each = 2)
...
line_count <- 
  c(
    2, 3, 3, 4, 6, 10, # tidySeurat
    2, 3, 3, 4, 6, 10, # tidySingleCellExperiment
    2, 3, 5, 6, 7, 10, # tidySpatialExperiment
    3, 5, 4, 6, 2, 3, # tidySummarizedExperiment
...
plot_variable <-
  ...
  geom_line() +
  geom_point(size = 1) +
  facet_grid(rows = vars(metric), cols = vars(package), scale = "free") +
  scale_y_continuous(limits = c(0, 8), breaks = seq(0, 8, 2)) + 
  scale_colour_manual(values = c("#56B4E9", "#B14380")) +
  labs(x = "Operation", y = NULL, colour = "Method") +
  theme_multipanel +
  theme(
...
plot_grid(plot_variable, plot_line, plot_time, ncol = 1, align = "v", rel_heights = c(1, 1, 1.6))
```
...
# Benchmarking performed on a 2021 14-inch MacBook Pro, with an Apple M1 Max chip and 32 GB of memory 
