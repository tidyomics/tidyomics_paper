...
se_base_mean <- function() {
  se_object_counts <- assay(se_object, "counts")
  se_object_count_means <- rowMeans(se_object_counts)
  se_object_counts[se_object_count_means > 500, ]
}
```
...
bulk_tidy_plot <- function() {
  se_object_tidy_normalised |>
    ggplot(aes(counts_scaled + 1, group = .sample)) +
    geom_density() +
    scale_x_log10()
}
...
bulk_base_plot <- function() {
  plot(density(se_object_base_normalised[, 1]), type = "n")
...
bulk_base_variable <- 
  function() {
    ...
    for (i in 1:length(se_object_split)) {
      se_object_counts <- assay(se_object_split[[i]], "counts")
...
plot_grid(plot_variable, plot_line, plot_time, ncol = 1, align = "v", rel_heights = c(1, 1, 2))
```
